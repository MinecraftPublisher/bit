...
var require_assembler = __commonJS(() => {
  var assemble = function(code, nostart = false, expronly = false) {
    ...
    const readExpression = (readonly = false) => {
      ...
      if (startsWith("r")) {
        ...
        if (letters.includes(letter)) {
          ...
          if (bin_rep.length < 28)
            bin_rep = new Array(28 - bin_rep.length).fill("0").join("") + bin_rep;
          skipWhitespace();
          while (startsWith(","))
            skipWhitespace();
          if (readonly)
        ...
        if (bin.length < 29)
          bin = new Array(29 - bin.length).fill("0").join("") + bin;
        skipWhitespace();
        while (startsWith(","))
          skipWhitespace();
        if (readonly)
    ...
    while (pointer < code.length) {
      ...
      if (startsWith("#")) {
        ...
        skipWhitespace();
        let op = startsWith("nand") || startsWith("N") || startsWith("!") ? "nand" : startsWith("add") || startsWith("+") || startsWith("plus") ? "add" : "unknown";
        if (op === "unknown") {
        ...
        let conds = stub().split(")")[0].replace(/( +or *)|( *or +)/g, " ").replace(/ +/g, " ").replace(/0|==?/g, "zero").replace(/\>|\+1?/g, "more").replace(/\-1?|\</g, "less").split(" ");
        conds = [...new Set(conds)];
        const flags = (conds.includes("zero") ? "1" : "0") + (conds.includes("less") ? "1" : "0") + (conds.includes("more") ? "1" : "0");
        ...
        skipWhitespace();
        let macro_code = ``;
        while (!startsWith(`end ${macro_name}`))
          macro_code += code[pointer++];
        ...
        for (let i = 0;i < macro.argcount; i++)
          if (macro.args[i] != "")
            __code = __code.replace(new RegExp(macro.args[i], "g"), macro_args[i]);
  ...
  var output = assemble(fs.readFileSync(process.argv[2], "utf-8")).trim();
  if (process.argv.includes("--minify"))
    console.log(output.replace(/[^01]/g, "").replace(/\n    /g, "\n"));
  else
    console.log(output.replace(/\n    /g, "\n"));
});
export default require_assembler();
